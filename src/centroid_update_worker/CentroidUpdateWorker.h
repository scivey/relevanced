...
class CentroidUpdateWorkerIf {
 public:
  ...
  virtual folly::Future<folly::Try<std::string>>
    joinUpdate(const std::string &) = 0;
  ...
  virtual folly::Future<folly::Try<bool>>
    update(const std::string &centroidId) = 0;
...
class CentroidUpdateWorker : public CentroidUpdateWorkerIf {
 ...
 public:
  CentroidUpdateWorker(
    std::shared_ptr<CentroidUpdaterFactoryIf>,
    std::shared_ptr<wangle::FutureExecutor<wangle::CPUThreadPoolExecutor>>
  );
  ...
  folly::Future<folly::Try<std::string>>
    joinUpdate(const std::string&) override;
  ...
  folly::Future<folly::Try<bool>>
    update(const std::string &centroidId) override;
  ...
  std::shared_ptr<util::Debouncer<std::string>>
    debug_getUpdateQueue();
