 ...
 * Thrift protocol.
 *
 * A major aspect of this is translating wrapped exceptions like
 *`CentroidDoesNotExist`
 * and `DocumentDoesNotExist` into corresponding status codes for the client,
 *e.g.
...
class ThriftRelevanceServer : public thrift_protocol::RelevancedSvIf {
 ...
 public:
  ...
  folly::Future<std::unique_ptr<thrift_protocol::SimilarityResponse>>
  ...
  folly::Future<bool> future_joinCentroid(
      std::unique_ptr<std::string> centroidId) override;
