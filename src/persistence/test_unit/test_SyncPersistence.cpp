...
TEST(SyncPersistence,
  ...
  dbHandle.createNewCentroid("c1");
  dbHandle.createNewCentroid("c2");
  dbHandle.createNewCentroid("c3");
  ...
  for (auto id: docIds) {
    ProcessedDocument doc(id, vector<ScoredWord>{}, 0.0);
    dbHandle.saveDocument(&doc);
  }
  dbHandle.addDocumentToCentroid("c1", "doc-1");
  dbHandle.addDocumentToCentroid("c3", "doc-1");
  dbHandle.addDocumentToCentroid("c2", "doc-3");
  dbHandle.addDocumentToCentroid("c1", "doc-4");
  dbHandle.addDocumentToCentroid("c3", "doc-6");
  dbHandle.addDocumentToCentroid("c2", "doc-6");
  dbHandle.addDocumentToCentroid("c2", "doc-7");

  vector<string> expected {
    "doc-2", "doc-5", "doc-8"
  };
  auto result = dbHandle.listUnusedDocuments(10);
  EXPECT_EQ(expected, result);

  vector<string> expectedShort {
    "doc-2", "doc-5"
  };
  auto shortResult = dbHandle.listUnusedDocuments(2);
  EXPECT_EQ(expectedShort, shortResult);
}
